...
def top_p_attention(graph: DGLHeteroGraph, attn_scores: Tensor, p: float = 0.75):
    ...
    def top_p_reduce_func(nodes):
        edge_attention_score = nodes.mailbox['m_a']
        batch_size, neighbor_num, head_num, _ = edge_attention_score.shape
        sorted_attns, sorted_indices = torch.sort(edge_attention_score, descending=True, dim=1)
        cumulative_attns = torch.cumsum(sorted_attns, dim=1)
        neighbors_to_remove = cumulative_attns > p
        ...
        sorted_attns = sorted_attns.squeeze(-1).transpose(1, 2).contiguous().view(batch_size * head_num, neighbor_num)
        ...
        ret_a = sorted_attns[row_arrange_idx, top_p_score_idx].view(batch_size, head_num, 1)
        cumulative_attns = cumulative_attns.squeeze(-1).transpose(1, 2).contiguous().view(batch_size * head_num,
                                                                                          neighbor_num)
        ret_a_sum = cumulative_attns[row_arrange_idx, top_p_score_idx].view(batch_size, head_num, 1)
        return {'top_a': ret_a, 'top_as': ret_a_sum}
...
def top_kp_attn_normalization(graph: DGLHeteroGraph, attn_scores: Tensor, attn_mask: Tensor, topk_sum: Tensor):

    def edge_udf_attn_normalization_func(edges):
        attention_scores = edges.data['ta']
        sum_attn_scores = edges.dst['attn_sum']
        norm_attns = attention_scores/sum_attn_scores
        return {'norm_attn': norm_attns}

    with graph.local_scope():
        graph.edata['ta'] = attn_scores
        graph.edata['ta'][~attn_mask] = 0.0
        graph.dstdata['attn_sum'] = topk_sum
