...
class RGDTLayer(nn.Module):
    ...
    def forward(self, graph, ent_feat: Tensor, rel_feat: Tensor, get_attention=False):
        ...
        with graph.local_scope():
            ...
            # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            graph.srcdata.update({'ft': feat_head})
            graph.dstdata.update({'fd': feat_tail})
            graph.apply_edges(fn.u_mul_v('ft', 'fd', 'e'))
            e = self.attn_activation(graph.edata.pop('e') * feat_rel)
